defmodule Christine.Typechecker do
      ...
      case List.keyfind(e.ctx, name, 0) do
        ...
        nil ->
          case find_def(e, name) do
            %AST.Fixpoint{domain: ty} when ty != nil ->
              ty
  ...
  def infer(%Env{} = e, %AST.Constr{index: j, inductive: d, args: args}) do
    case Enum.find(d.constrs, fn {idx, _, _} -> idx == j end) do
      ...
      _ ->
        {:error, {:constructor_not_found, j, d.name}}
    end
  end
    ...
    if fuel <= 0 do
      ...
      if structural_equal?(e, n1, n2) do
        ...
        case {n1, n2} do
          ...
          {%AST.Pi{name: x1, domain: d1, codomain: c1},
           %AST.Pi{name: _x2, domain: d2, codomain: c2}} ->
  ...
  def structural_equal?(e, t1, t2) do
    case {t1, t2} do
      {%AST.Var{name: n1}, %AST.Var{name: n2}} ->
        AST.names_match?(n1, n2)

      {%AST.Universe{level: l1}, %AST.Universe{level: l2}} ->
        l1 == l2
      ...
      {%AST.Inductive{name: n1}, %AST.Inductive{name: n2}} ->
        AST.names_match?(n1, n2)
      ...
      {%AST.Number{value: v1}, %AST.Number{value: v2}} ->
        v1 == v2

      {%AST.String{value: v1}, %AST.String{value: v2}} ->
        v1 == v2
  ...
  def normalize(e, t) do
    ...
    case red do
        ...
        case n_f do
          ...
          %AST.Var{name: vname} ->
            ...
            case find_def(e, vname) do
              %AST.Lam{} = lam -> normalize(e, %AST.App{func: lam, arg: n_arg})
      ...
      %AST.Lam{name: x, domain: a, body: b} ->
        %AST.Lam{
          name: x,
          domain: normalize(e, a),
          body: normalize(%{e | ctx: [{x, a} | e.ctx]}, b)
        }
      ...
      %AST.Pi{name: x, domain: a, codomain: b} ->
        %AST.Pi{
          name: x,
          domain: normalize(e, a),
          codomain: normalize(%{e | ctx: [{x, a} | e.ctx]}, b)
        }
      ...
      %AST.Var{name: vname} = var ->
        # If this Var is a known constructor in defs, canonicalize it.
  ...
  defp do_reduce(e, %AST.App{func: f, arg: arg}, fuel) do
    ...
    case f_red do
      ...
      %AST.Fixpoint{args: applied_args} = fix ->
        # Don't reduce arg yet, preserve for potential unfolding
  ...
  defp do_reduce(e, %AST.Ind{inductive: ind_def, cases: cases, term: term} = ind, fuel) do
    ...
    case target do
      %AST.Constr{index: j, args: args} ->
        ...
        ind_full =
          if is_nil(ind_def.constrs) or ind_def.constrs == [] do
            Map.get(e.env, ind_def.name) ||
              Enum.find_value(e.env, fn {n, ind} ->
                if AST.names_match?(n, ind_def.name), do: ind
              end) || ind_def
          else
            ind_def
          end
      ...
      %AST.Number{value: v} ->
        reduce(e, %{ind | term: unfold_number(e, v)}, fuel - 1)

      _ ->
        # When the match term is App(Var(ctor), args) or even just Var(ctor_name),
  ...
  defp do_reduce(e, %AST.Var{name: name} = t, fuel) do
    ...
    case List.keyfind(e.ctx, name, 0) do
      {^name, _} ->
        # Ctx-bound names are opaque (Pi-introduced variables, local hypotheses,
        # constructors registered via add_constructors_to_ctx, etc.).
        t
  ...
  # proof terms and recursive defs opaque when they appear in ctx.
  ...
  defp find_def(e, name) do
    ...
    # so that `Import Coq` overrides Prelude definitions registered under bare names.
    case Map.get(e.name_to_mod, name) do
      nil ->
        Map.get(e.defs, name)
      ...
      mod ->
        prefix = if mod == "local", do: "", else: mod <> "."
        Map.get(e.defs, prefix <> name) || Map.get(e.defs, name)
    end
  end

  # Search defs for a constructor by short name using suffix matching.
  ...
  # Constr — because constructor Vars are opaque in ctx — this function peels
  # the App chain and resolves the root Var as a Constr application.
  # Returns {:ok, Constr} if successful, :error otherwise.
  defp try_resolve_ctor_app(e, term) do
    {root, args} = peel_app_chain(term)

    case root do
      %AST.Var{name: name} ->
        val = find_def(e, name) || find_ctor_def(e, name)
        ...
        case val do
          ...
          v ->
            if constructor_def?(v) do
              # Apply the Lam (constructor wrapper) to each arg to get Constr
              result =
                Enum.reduce(args, v, fn arg, acc ->
                  case acc do
                    %AST.Lam{name: x, body: body} -> subst(x, arg, body)
                    %AST.Constr{args: a} = c -> %AST.Constr{c | args: a ++ [arg]}
                    _ -> %AST.App{func: acc, arg: arg}
                  end
                end)

              case result do
                %AST.Constr{} -> {:ok, result}
                _ -> :error
              end
            else
              :error
  ...
  # Peel App chain: App(App(..App(f, a1), a2)..., an) → {f, [a1, a2, ..., an]}
  defp peel_app_chain(term, acc \\ []) do
    case term do
      %AST.App{func: f, arg: a} -> peel_app_chain(f, [a | acc])
      _ -> {term, acc}
    end
  end
    ...
    if n_args >= n_params and n_params > 0 do
      ...
      unfolded =
        if n_args > n_params do
          Enum.reduce(Enum.drop(args, n_params), unfolded_base, fn a, acc ->
            %AST.App{func: acc, arg: a}
          end)
        else
          unfolded_base
        end
  ...
  defp collect_lams(%AST.Lam{name: x, body: b}),
  ...
  defp do_apply_args(e, f, [arg | rest], ty_sig, %AST.Ind{} = ind, n_skip) do
    ...
    if n_skip > 0 do
      ...
      is_recursive =
        case normalize(e, arg_domain) do
          %AST.Var{name: n} ->
            AST.names_match?(n, ind.inductive.name)

          %AST.Inductive{name: n} ->
            AST.names_match?(n, ind.inductive.name)
      ...
      if is_recursive do
          ...
          case normalize(e, arg_domain) do
            %AST.Pi{name: yn_raw, domain: yd} ->
              ...
              %AST.Lam{
                name: yn,
                domain: yd,
                body: %AST.Ind{ind | term: %AST.App{func: arg, arg: %AST.Var{name: yn}}}
              }
        ...
        # binder). Coq-style fixpoints use direct recursion so their case body is
        # just Lam(k, body) with no ih_k binder. Only inject ih if it will be consumed.
        applied_arg = reduce(e, %AST.App{func: f, arg: arg}, 1000)

        if match?(%AST.Lam{}, applied_arg) do
          do_apply_args(e, %AST.App{func: applied_arg, arg: ih}, rest, updated_next_sig, ind, 0)
        else
          do_apply_args(e, applied_arg, rest, updated_next_sig, ind, 0)
        end
      else
  ...
  def subst(x, v, t) do
    case t do
      ...
      %AST.App{func: f, arg: arg} ->
        %AST.App{func: subst(x, v, f), arg: subst(x, v, arg)}

      %AST.Lam{name: n, domain: d, body: b} ->
        if n == x,
          do: %AST.Lam{name: n, domain: subst(x, v, d), body: b},
          else: %AST.Lam{name: n, domain: subst(x, v, d), body: subst(x, v, b)}

      %AST.Pi{name: n, domain: d, codomain: c} ->
        if n == x,
          do: %AST.Pi{name: n, domain: subst(x, v, d), codomain: c},
      ...
      %AST.Ind{inductive: d, motive: p, cases: cases, term: tm} ->
        %AST.Ind{
          inductive: d,
          motive: subst(x, v, p),
          cases: Enum.map(cases, &subst(x, v, &1)),
          term: subst(x, v, tm)
        }
      ...
      %AST.Fixpoint{name: n, domain: d, body: b, args: args} ->
        if n == x,
          do: t,
            ...
            domain: subst(x, v, d),
            body: subst(x, v, b),
            args: Enum.map(args, &subst(x, v, &1))
          }
      ...
      %AST.Let{decls: decls, body: b} ->
        ...
        if Enum.any?(decls, fn {n, _} -> n == x end),
          do: %AST.Let{decls: new_decls, body: b},
  ...
  @doc """
  Simultaneously substitute multiple variables in a term using a bindings map.
  Unlike sequential application of individual `subst` calls, this does a single
  traversal so that bindings like {A→B, B→A} give the expected result (Var(A)→B,
  not Var(A)→B→A due to cascading).
  """
  def subst_simultaneous(bindings, t) when map_size(bindings) == 0, do: t

  def subst_simultaneous(bindings, t) do
    case t do
      %AST.Var{name: n} ->
        Map.get(bindings, n, t)

      %AST.App{func: f, arg: arg} ->
        %AST.App{func: subst_simultaneous(bindings, f), arg: subst_simultaneous(bindings, arg)}

      %AST.Lam{name: n, domain: d, body: b} ->
        new_bindings = Map.delete(bindings, n)

        %AST.Lam{
          name: n,
          domain: subst_simultaneous(bindings, d),
          body: subst_simultaneous(new_bindings, b)
        }
      ...
      %AST.Pi{name: n, domain: d, codomain: c} ->
        ...
        %AST.Pi{
          name: n,
          domain: subst_simultaneous(bindings, d),
          codomain: subst_simultaneous(new_bindings, c)
        }

      %AST.Ind{inductive: d, motive: p, cases: cases, term: tm} ->
        %AST.Ind{
          inductive: d,
          motive: subst_simultaneous(bindings, p),
          cases: Enum.map(cases, &subst_simultaneous(bindings, &1)),
          term: subst_simultaneous(bindings, tm)
        }
      ...
      %AST.Fixpoint{name: n, domain: d, body: b, args: args} ->
        ...
        %AST.Fixpoint{
          ...
          domain: subst_simultaneous(bindings, d),
          body: subst_simultaneous(new_bindings, b),
          args: Enum.map(args, &subst_simultaneous(bindings, &1))
        }
      ...
      %AST.Let{decls: decls, body: b} ->
        bound = MapSet.new(Enum.map(decls, &elem(&1, 0)))
        new_bindings = Map.drop(bindings, MapSet.to_list(bound))
        new_decls = Enum.map(decls, fn {ln, lv} -> {ln, subst_simultaneous(bindings, lv)} end)
        %AST.Let{decls: new_decls, body: subst_simultaneous(new_bindings, b)}
  ...
  def unwrap_eq(term) do
    case term do
      %AST.App{} = term ->
        ...
        is_eq =
          head_name &&
            (AST.names_match?("eq", head_name) or AST.names_match?("Eq", head_name) or
               head_name == "Coq.Init.Logic.eq")
